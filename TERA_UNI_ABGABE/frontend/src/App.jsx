...
export default function App() {
  ...
  async function analyze(name) {
    ...
    try {
      ...
      const res = await fetch('http://141.100.238.104:8080/api/analysis/analyze', {
        method: 'POST',
      ...
      setTimeout(async () => {
        try {
          ...
          const hexRes = await fetch(`http://141.100.238.104:8080/api/analysis/risk-map?city=${encodeURIComponent(name)}&resolution=8`, { signal })
          // Professional Analysis (parallel)
          fetch(`http://141.100.238.104:8080/api/analysis/professional?city=${encodeURIComponent(name)}`)
